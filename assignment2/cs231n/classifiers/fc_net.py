...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            if self.use_batchnorm:
                layer_input, layer_cache[i] = affine_bn_relu_forward(
                    layer_input, self.params['W%d'%(i+1)], self.params['b%d'%(i+1)],
                    self.params['gamma%d'%(i+1)], self.params['beta%d'%(i+1)], self.bn_params[i])
            else:
                layer_input, layer_cache[i] = affine_relu_forward(
                    layer_input, self.params['W%d'%(i+1)], self.params['b%d'%(i+1)])
            if self.use_dropout:
                layer_input, dropout_cache[i] = dropout_forward(layer_input, self.dropout_param)
        scores, layer_cache[self.num_layers - 1] = affine_forward(
            layer_input, self.params['W%d'%(self.num_layers)], self.params['b%d'%(self.num_layers)])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dout, dw, db = affine_backward(dout, layer_cache[self.num_layers - 1])
        grads['W%d'%(self.num_layers)] = dw + self.reg * self.params['W%d'%(self.num_layers)]
        grads['b%d'%(self.num_layers)] = db
        for i in range(self.num_layers - 2, -1, -1):
            loss = loss + 0.5 * self.reg * np.sum(np.square(self.params['W%d'%(i+1)]))
            if self.use_dropout:
